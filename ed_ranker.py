    ...
    def predict(self, data, dynamic_option, order_learning):
        ...
        for batch in data:  # each document is a minibatch
            ...
            if order_learning:
                ...
                pred_scores = [(m['raw']['mention'], {m['selected_cands']['named_cands'][i]: score[i] if m['selected_cands']['mask'][i] == 1
                                 else -1
            ...
            if self.seq_len>0 and self.one_entity_once:
                ...
                for dname, entity in zip(doc_names, pred_scores):
                    predictions_score[dname].append({'mention': entity[0], 'pred': (entity[1], 0.)})
